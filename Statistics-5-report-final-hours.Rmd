...
```{r exploratory plots, cache=TRUE}
...
```{r orig Models With refined Correlation and refined interactions}
modelString = "
model {
for ( i in 1:Ntotal ) {
...
```{r sens Models With refined Correlation and refined interactions}
modelString.sens = "
model {
for ( i in 1:Ntotal ) {
	y[i,1] ~ dnorm( beta[pp[i],1] + beta[pp[i],2]*x[i, 3]+beta2[pp[i]]*x[i,2]+ sum( betax[1:Nx0] * x[i,(Nx-Nx0+1):Nx]), 1/sigma^2 )
...
load("summary.COR.original.rdata")
load("summary.COR.sensitivity.rdata")
load("summary.COR2.original.rdata")
load("summary.COR2.sensitivity.rdata")

apa_table(x = sum.1, caption = "The first model", note = "This table contains statistics pertaining to our replication of the original adjusted models.", landscape = TRUE, small = TRUE)
apa_table(x = sum.2, caption = "The Second model", note = "This table contains statistics pertaining to our replication of the original adjusted models.", landscape = TRUE, small = TRUE)
apa_table(x = sum.sens.1, caption = "The first model", note = "This table contains statistics pertaining to our replication of the original adjusted models.", landscape = TRUE, small = TRUE)
apa_table(x = sum.sens.2, caption = "The first model", note = "This table contains statistics pertaining to our replication of the original adjusted models.", landscape = TRUE, small = TRUE)
