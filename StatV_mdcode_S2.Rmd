...
dataList = list(
    ...
    Ntotal = Ntotal ,
    Nx = dim(x)[2] ,
    NxO = dim(x)[2]-1, #employment is measured at the individual level
    Nsubjects = Nsubjects 
  )
...
out.simple.model <- run.jags(model = "model_full_no_interaction", monitor = c("beta0mu", "beta1mu", "beta2mu","beta3mu","beta4mu", "beta"), data = simple_model, inits = myinits, n.chains = nchains, adapt = nadapt, burnin = nburn, sample = niter)
