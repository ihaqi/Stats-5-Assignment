...
yName = "norm.LS" 
xName = c("norm.a","norm.h","norm.p","norm.e")
y1 <- data.norm[1]
y <- as.matrix(y1)
...
model {
for ( i in 1:Ntotal ) {
	y[i,1] ~ dnorm( beta0[pp[i]] + beta1[pp[i]]*x[i,1]+ beta2[pp[i]]*x[i, 2]+beta3[pp[i]]*x[i,3]+beta4[pp[i]]*x[i,4] + sum( betax[1:Nx0] * x[i,(Nx-Nx0+1):Nx] ), 1/sigma^2 )
} 
...
out.simple.model <- run.jags(model = "model_full_no_correlation", monitor = c("beta0mu", "beta1mu", "beta2mu","beta3mu","beta4mu", "beta"), data = dataList, inits = myinits, n.chains = nchains, adapt = nadapt, burnin = nburn, sample = niter)
...
```{r }
parametersI = c( "sigma", "beta","beta0mu","beta0sigma", "beta1mu","beta1sigma","beta2mu","beta2sigma", "beta3mu","beta3sigma","beta4mu","beta4sigma", "betax") 
...
save(out.simple.modelI, file = "simple.model1.Rdata")
```
