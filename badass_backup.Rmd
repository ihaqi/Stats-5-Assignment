...
beta3[j] ~ dnorm( beta3mu , 1/beta3sigma^2 )
 ...
 # Priors vague :
    ...
    varprior <- dgamma(0.00001, 0.00001)
...
```{r Model with refined interactions}
#Preperation
x = as.matrix(data.norm[3:6],ncol=length(xName))
Nx = dim(x)[2]
Nx0 = dim(x)[2]-3 #Change for between subject madness
...
out.model.I.noCOR <- run.jags(model = "model_full_no_correlation_refinedI", monitor = c("beta0mu", "beta1mu", "beta2mu","beta3mu", "betax"), data = dataList, inits = myinits, n.chains = nchains, adapt = nadapt, burnin = nburn, sample = niter, thin = 1)
